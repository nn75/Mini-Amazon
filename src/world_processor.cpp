...
using namespace std;
WorldProcessor::WorldProcessor(message_queue<pair<long int, ACommands> >& mq1, message_queue<AResponses>& mq2, message_queue<AUCommands>& mq3,WorldCommunicator* world_communicator, long int& wnum, long int &unum, mutex & mt):
    ...
    // this->world_thread = thread(&WorldProcessor::world_command_process, this);
}
        ...
        if(!recv_world_queue.if_empty()){
            ...
            if(tmp_msg.ready_size()!=0){
                for(int i =0; i < tmp_msg.ready_size();i++){
                    ...
                    ack_res.add_acks(ship_seq);
                    // please fill database code here use ship_id to select truck_id and warehouse number
                    // also update the status of package
                    // if(truckid > 0){
                    //     ACommands load_msg;
