        ...
        if (!recv_world_queue.if_empty()) {
            ...
            if (tmp_msg.loaded_size() != 0) {
                for (int i = 0; i < tmp_msg.loaded_size(); i++) {
                    ...
                    // database: if world said order is loaded, update order
                    ...
                    string update_to_packed =
                        "UPDATE orders_order SET status = delivering WHERE "
                        "tracking_number= " +
                        to_string(ship_id) + ";";
                    work W(C);
                    ...
                    pair<long int, AUCommands> ups_deliver_pair(
                        ups_seqnum, ups_deliver_msg);
                    ups_seqnum++;
                    mtx.unlock();  /////unlock
            ...
            if (tmp_msg.ready_size() != 0) {
                for (int i = 0; i < tmp_msg.ready_size(); i++) {
                    ...
                    // database: if world said the order is packed, update order
                    // status to "packed"
                    connection C(
                        "dbname = mini_amazon user = postgres password = "
                        "passw0rd hostaddr = 67.159.95.41 port = 5432");
                    ...
                    // check truck if truck_num != -1, send load message to
                    // world
                    string check_truck =
                        "SELECT orders_order WHERE tracking_number = " +
                    ...
                    if (R.size() == 1) {
                        ...
                        string update_to_loading =
                            "UPDATE orders_order SET status = loading WHERE "
                            "tracking_number= " +
                            to_string(ship_id) + ";";
                        work W2(C);
